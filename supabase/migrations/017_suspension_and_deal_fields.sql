...
CREATE UNIQUE INDEX IF NOT EXISTS startup_views_daily_idx
  ON startup_views (startup_id, investor_id, (viewed_at::date));
...
CREATE POLICY "deals_not_suspended_insert" ON deals FOR INSERT
  WITH CHECK (NOT is_suspended());

DROP POLICY IF EXISTS "messages_not_suspended_insert" ON messages;
CREATE POLICY "messages_not_suspended_insert" ON messages FOR INSERT
  WITH CHECK (NOT is_suspended());

DROP POLICY IF EXISTS "contracts_not_suspended_insert" ON contracts;
CREATE POLICY "contracts_not_suspended_insert" ON contracts FOR INSERT
  WITH CHECK (NOT is_suspended());
