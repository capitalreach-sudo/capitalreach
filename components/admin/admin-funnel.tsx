  ...
  return (
    <section className="border border-cr-p4 rounded-xl p-5 mb-6">
      <div className="flex items-baseline justify-between gap-3 flex-wrap mb-4">
        <h2 className="font-bold text-cr-ink">{t("funnel.title")}</h2>
        ...
        {steps.map((s) => (
          <div key={s.key} className="flex items-center gap-3">
            ...
            <div className="flex-1 bg-cr-p3 rounded-full h-2.5 overflow-hidden min-w-[60px]">
              <div
                className={`h-full rounded-full ${s.key === worst?.key ? "bg-red-400" : "bg-cr-copper"}`}
            ...
            <span className="text-[11px] text-cr-i4 w-14 text-right shrink-0">
