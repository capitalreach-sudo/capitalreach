...
const MICRO_LABEL: React.CSSProperties = {
  // The one caps voice: 11px/500/0.08em ink-3 (mono stays mono).
  fontFamily: "'JetBrains Mono', monospace", fontWeight: 500, fontSize: "11px",
  letterSpacing: "0.08em", textTransform: "uppercase", color: "var(--cr-ink-3)",
};
      ...
      <div className="max-w-[1200px] mx-auto px-6 md:px-10 py-16 md:py-24 grid lg:grid-cols-2 gap-12 items-center">
        ...
        <div style={{ background: "var(--cr-paper-2)", border: "1px solid var(--cr-rule)", borderRadius: "6px", overflow: "hidden" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: "12px", padding: "12px 16px", borderBottom: "1px solid var(--cr-rule)" }}>
            ...
            <span style={{ display: "inline-flex", alignItems: "center", gap: "8px", fontFamily: "'JetBrains Mono', monospace", fontWeight: 500, fontSize: "11px", textTransform: "uppercase", letterSpacing: "0.08em", color: "var(--cr-ink-3)", textAlign: "end" }}>
              {reading !== null && !failed && (
                <span className="animate-pulse" style={{ width: 6, height: 6, borderRadius: "50%", background: "var(--cr-ink-4)", display: "inline-block", flexShrink: 0 }} aria-hidden />
              )}
              {stageLabel} · {industry}
            </span>
          </div>
          ...
          {failed ? (
            ...
            <div style={{ padding: "24px", minHeight: "172px", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: "12px", textAlign: "center" }}>
              <p style={{ fontFamily: "'DM Sans', sans-serif", fontWeight: 300, fontSize: "15px", color: "var(--cr-ink-3)", margin: 0, maxWidth: "36ch", lineHeight: 1.6 }}>
                {tf("match.unavailable", "Live figures are unavailable right now.")}
              </p>
              <button
            ...
            >
              <div style={{ fontFamily: "'JetBrains Mono', monospace", fontWeight: 700, fontSize: "clamp(56px, 9vw, 92px)", color: "var(--cr-copper)", lineHeight: 1, fontVariantNumeric: "tabular-nums" }}>
                {/* 28px: the top step of the fixed scale -- the placeholder is
                    not the lead figure, so it does not borrow the clamp. */}
                {reading !== null ? <CountUp value={reading.count} /> : <span style={{ color: "var(--cr-ink-4)", fontSize: "28px" }}>···</span>}
              </div>
              ...
              {reading !== null && (
                <p style={{ fontFamily: "'DM Sans', sans-serif", fontWeight: 300, fontSize: "15px", color: "var(--cr-ink-3)", marginTop: "12px", marginBottom: 0 }}>
          ...
          {reading !== null && (reading.listings !== undefined || reading.raising !== undefined) && (
            <div style={{ display: "grid", gridTemplateColumns: reading.listings !== undefined && reading.raising !== undefined ? "1fr 1fr" : "1fr", borderTop: "1px solid var(--cr-rule)" }}>
              {reading.listings !== undefined && (
                <div style={{ background: "var(--cr-paper-2)", padding: "12px 16px", textAlign: "center" }}>
                  ...
                  <div style={{ fontFamily: "'DM Sans', sans-serif", fontWeight: 500, fontSize: "11px", color: "var(--cr-ink-3)", textTransform: "uppercase", letterSpacing: "0.08em", marginTop: "8px" }}>
