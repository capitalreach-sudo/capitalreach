...
import { FeePortal } from "@/components/startup/fee-portal";
import { getLocale, getTranslator } from "@/lib/locale-server";

/**
 * The success-fee portal: what this founder owes, what they have paid, and what
 * an unpaid fee costs and when.
 *
 * Server component so the bounce happens before anything renders. The fee is a
 * private financial obligation of one company; the wrong person must never see
 * the shape of the page, let alone its contents. The route behind it re-checks
 * ownership on its own -- neither check trusts the other.
 */
export default async function StartupFeesPage() {
  ...
  // Ownership is proven by the owner_id filter; the service role sees past the
  // column grants migration 109 revoked from client keys.
      ...
      <main style={{ background: "var(--cr-paper)", minHeight: "100vh", paddingBottom: "64px" }}>
        <div style={{ maxWidth: "672px", margin: "0 auto", padding: "40px 24px" }}>
          ...
          <p style={{
            ...
            color: "var(--cr-ink-4)", marginBottom: "8px",
          }}>
            {startup.name}
          </p>
          ...
          <p style={{
            fontFamily: "'DM Sans', sans-serif", fontWeight: 300, fontSize: "13px",
            lineHeight: 1.65, color: "var(--cr-ink-3)", maxWidth: "60ch", marginBottom: "32px",
          }}>
            {t("feePortal.subtitle")}
          </p>
